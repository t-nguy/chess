...
void Game::mouseDown( ImVec2& location, Entity* entity )
{
    ...
    ChessSquare &src = static_cast<ChessSquare &>(*(_dragBit->getHolder()));

	int square = src.getRow() * 8 + src.getColumn();

	for(int i = 0; i < _mg->moveCount; i++){
		if(square == _mg->getSrc(_mg->moveList[i])){
			int dst = _mg->getDst(_mg->moveList[i]);
			ChessSquare &temp = static_cast<ChessSquare &>(getHolderAt(dst % 8, dst / 8));
			temp.setMoveHighlighted(true);
		}
	}
...
void Game::clearBoardHighlights()
{
	for(int i = 0; i < 8; i++){
		for(int j = 0; j < 8; j++){
			ChessSquare &temp = static_cast<ChessSquare &>(getHolderAt(i, j));
			temp.setMoveHighlighted(false);
		}
	}
}